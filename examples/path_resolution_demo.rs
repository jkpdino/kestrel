...
fn main() {
    ...
    println!("Setting up class hierarchy:");
    println!("  Root");
    ...
    let graphics = create_class("Graphics", 0..100, Visibility::Public, &root);
    add_typed_behavior(&graphics, vec!["Graphics".to_string()]);

    // Create Graphics.Shape
    let shape = create_class("Shape", 10..80, Visibility::Public, &root);
    ...
    let circle = create_class("Circle", 20..60, Visibility::Public, &root);
...
fn create_class(
    ...
    let class_name = Name::new(name.to_string(), span.clone());
