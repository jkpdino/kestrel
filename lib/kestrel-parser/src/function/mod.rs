...
fn function_declaration_parser_internal() -> impl Parser<Token, FunctionDeclarationData, Error = Simple<Token>> + Clone {
    ...
    visibility_parser_internal()
        ...
        .then(
            skip_trivia()
                .ignore_then(just(Token::Fn).map_with_span(|_, span| span))
        )
        .then(
            skip_trivia()
...
fn emit_function_declaration(sink: &mut EventSink, data: FunctionDeclarationData) {
    ...
    // Emit fn keyword
    sink.add_token(SyntaxKind::Fn, data.fn_span);
    ...
    sink.add_token(SyntaxKind::LBrace, data.lbrace);
    // TODO: Function body contents will be added later
    sink.add_token(SyntaxKind::RBrace, data.rbrace);
